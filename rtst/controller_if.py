...
__date__ = "$DateTime: 2020/10/21 14:57:59 $"
...
class ControllerInterface:
	...
	# Trackpad Z threshold for trouch detection
	...
	def trackpad_set_debug_mode(self, mode):
	...
	# IMU Calibration (not persistent yet)
	...
	# IMU Get temp
	#  Sets IMU int a goofy debug mode
	...
	## Thumbstick 
	##########################################################################################################
	def set_thumbstick_raw(self, on):
	...
	def thumbstick_get_cal(self, side):
		...
		(side, x_center_min, x_center_max, x_full_min, x_full_max, y_center_min, y_center_max, y_full_min, y_full_max) = struct.unpack('=B8H', report_bytes)
	#	print('Joytick Cal Side: {}, X_C_MIN: {}, X_C_MAX: {}, X_F_MIN: {}, X_F_MAX: {}, Y_C_MIN: {}, Y_C_MAX: {}, Y_F_MIN: {}, Y_F_MAX: {}'.format(side, x_center_min, x_center_max, x_full_min, x_full_max, y_center_min, y_center_max, y_full_min, y_full_max))
	...
	def thumbstick_set_cal(self, side, x_center_min, x_center_max, x_full_min, x_full_max, y_center_min, y_center_max, y_full_min, y_full_max):
		op = 0xDA
		report_bytes = struct.pack('=B8H', side, x_center_min, x_center_max, x_full_min, x_full_max, y_center_min, y_center_max, y_full_min, y_full_max)
	...
	def pressure_get_cal(self, side):
		...
		(side, max, min) = struct.unpack('=B2H', report_bytes)
		print('Pressure Cal Side: {}, MAX: {}, MIN: {}'.format(side, max, min))
		return side, max, min
		
	def pressure_set_cal(self, side, max, min):
		op = 0xE1
		# left = 0, right = 1
		report_bytes = struct.pack('=B2H', side, max, min)
...
#		return (uid_0, uid_1)
    ...
    ##########################################################################################################
	def get_raw_trackpad_data(self, side):
		if side == self.SIDE_LEFT:
			self.trackpad_set_debug_mode(0x01)
		elif side == self.SIDE_RIGHT:
			self.trackpad_set_debug_mode(0x02)

		sleep(.2)  # Let that command process
		data = self.get_data()

		self.trackpad_set_debug_mode(0x00)
		return (data['y_raw'], data['x_raw'] )
	...
	def print_pin_test_errors(self):
		...
		for side in range(0, 2):
			for port in range(0, 2):
				...
				if pindata[0] != 0:
					print('Pins stuck at "1" Side:{} '.format('R' if side else 'L'))
					print( self.make_pin_name( port, pindata[0]))

				if pindata[1] != 0:
					print('Pins stuck at "0" Side:{} '.format('R' if side else 'L'))
					print( self.make_pin_name( port, pindata[1]))
				...
				for drive in range(0, 2):
					for other_port in range(0, 2):
						for pinset in range(0, 4):
							...
							for i in range(0, 8):
								if pindata[i] != 0:
									print('Pins Short Detected via walking {}): Side {}'.format('HI' if drive else 'LO', 'R' if side else 'L' ))
									print('\t' + self.make_pin_name_index(port, pinset * 8 + i ) + ' :: ' + self.make_pin_name(port, pindata[i])) 